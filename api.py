...
def fetchProfiles(authtoken, lat, long, myType='false', online='false', faceOnly='false', photoOnly='false', notRecentlyChatted='false'):
    geoHash = pygeohash.encode(lat, long, precision=12)
    url = f"https://grindr.mobi/v4/locations/{geoHash}/" \
          f"profiles?myType={myType}" \
          f"&online={online}" \
          f"&faceOnly={faceOnly}" \
          f"&photoOnly={photoOnly}" \
          f"&notRecentlyChatted={notRecentlyChatted}"
    x = requests.get(url, headers={'authorization': 'Grindr3 ' + authtoken})
    return json.loads(x.text)
...
class messageSocket:
    ...
    def authenticate(self):
        ...
        while i:
            ...
            if "features" in respons:
                self.ws.send('<auth mechanism="PLAIN" xmlns="urn:ietf:params:xml:ns:xmpp-sasl">' + generatePlainAuth(
                    self.tokens[0]) + '</auth>')

            if "success" in respons:
                self.ws.send('<open to="chat.grindr.com" version="1.0" xmlns="urn:ietf:params:xml:ns:xmpp-framing"/>')
                self.ws.send(
                    '<iq id="_bind_auth_2" type="set" xmlns="jabber:client"><bind xmlns="urn:ietf:params:xml:ns:xmpp-bind"><resource>' +
                    self.tokens[1] + '_web</resource></bind></iq>')
            ...
            if "_bind_auth_2" in respons:
                ...
                self.ws.send('<iq from="' + getProfileId(self.tokens[0]) + '@chat.grindr.com/' + self.tokens[
                    1] + '_web" id="' + str(
                    uuid.uuid4()) + ':carbons" type="set" xmlns="jabber:client"><enable xmlns="urn:xmpp:carbons:2"/></iq>')
    ...
    def message(self, id, message):
        ...
        data = {
            "sourceProfileId": str(getProfileId(self.tokens[0])),
            "targetProfileId": str(id),
            "messageId": str(uuid.uuid1()),
            "sourceDisplayName": str(getProfileId(self.tokens[0])),
            "type": "text",
            "timestamp": time.time(),
            "body": str(message)
        }
        ...
        data = data.replace('"', '&quot;')
        self.ws.send('<message from="' + getProfileId(self.tokens[0]) + '@chat.grindr.com" id="' + p + '" to="' + str(
            id) + '@chat.grindr.com" type="chat" xmlns="jabber:client"><body>' + data + '</body></message>')
    ...
    def tap(self, id, tapType):
        ...
        data = {"sourceProfileId": str(getProfileId(self.tokens[0])), "targetProfileId": str(id),
                "messageId": str(uuid.uuid1()), "sourceDisplayName": str(getProfileId(self.tokens[0])), "type": "image",
                "timestamp": time.time(), "body": str(body)}
        data = json.dumps(data)
        data = data.replace('"', '&quot;')
        self.ws.send(
            '<message from="' + getProfileId(self.tokens[0]) + '@chat.grindr.com" id="U2ot8EBFwLRAw6U9" to="' + str(
                id) + '@chat.grindr.com" type="image" xmlns="jabber:client"><body>' + data + '</body></message>')
